...
from src.data_cmp import PLProteinPeptideInteraction
...
def main(args):
    ...
    #     "neg_sampling_ratio_exp_{}".format(args.split_method)
    # )  # set the experiment
    mlflow.set_experiment(
        "{}_{}_{}".format(args.exp_name, args.split_method, args.perturb_method)
    )  # set the experiment
    ...
    with mlflow.start_run() as run:
        ...
        for _repeat_id in range(args.repeat_times):
            ...
            mlflow.log_metric(
                "test_auc_std_repoet_{}".format(_repeat_id), auc_std
            )
            mlflow.log_metric(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--perturb_ratio", type=int, default=0)
    parser.add_argument(
        "--perturb_method",
        type=str,
        required=True,
        choices=["add", "reduct", "mix"],
    )
